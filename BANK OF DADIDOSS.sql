...
INSERT INTO (nome, genero, biotipo, altura)
...
SELECT nome, desconto + 0.05 novo desconto FROM quarto WHERE desconto IS NOT MULL

SELECT nome, saida - chegada tempo FROM quarto

SELECT AVG(desconto), MAX(desconto), MIN(desconto) FROM quarto

SELECT MIN(descanso) * AVG(desconto) FROM quarto
...
SELECT h.nome, h.altura, p.peso FROM hospede h
	STTRAIGHT_JOIN peso p ON h.nome = p.nome

	SELECT h.nome, h.altura, p.peso FROM hos